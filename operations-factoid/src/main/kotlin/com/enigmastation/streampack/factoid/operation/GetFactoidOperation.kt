...
class GetFactoidOperation(val factoidService: FactoidService) : RouterOperation() {
    ...
    override fun handleMessage(message: RouterMessage): RouterMessage? {
        ...
        command?.let { cmd ->
            ...
            val selector = cmd.selector
            // get all attributes; we may not need them, but the common case would be yes, we do.
            val attributes = factoidService.findBySelector(cmd.selector)
            if (attributes.isNotEmpty()) {
                return when (cmd.attribute) {
                    ...
                    FactoidAttributeType.UNKNOWN -> {
                        // okay, we need to get the available factoid attributes, ordered by type.
                        message.respondWith(attributes.summarize(selector, factoidService))
                    }
...
fun List<FactoidAttribute>.summarize(selector: String, factoidService: FactoidService): String {
